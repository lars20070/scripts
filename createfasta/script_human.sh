...
mkdir gtdb
cd gtdb || exit 2
wget http://data.gtdb.ecogenomic.org/releases/latest/genomic_files_all/ar53_marker_genes_all.tar.gz
wget https://data.gtdb.ecogenomic.org/releases/latest/genomic_files_all/bac120_marker_genes_all.tar.gz
tar -zxvf ./ar53_marker_genes_all.tar.gz
tar -zxvf ./bac120_marker_genes_all.tar.gz
cd $homedir || exit 2
cat ./gtdb/ar53_marker_genes_all_r207/faa/*.faa ./gtdb/bac120_marker_genes_all_r207/faa/*.faa > gtdb.fasta
...
gzip -d ./gpd.faa.gz
mv ./gpd.faa ./gpd.fasta

# combine all sources
cat reference_hmp_dacc_gastrointestinal_tract.fasta reference_embl_bork_human_gut.fasta uniprot_sprot.fasta uniprot_trembl.fasta contaminants_crap.fasta ncbi_bacteria_archaea_fungi_viral.fasta uhgg.fasta gtdb.fasta gpd.fasta > combined.fasta
